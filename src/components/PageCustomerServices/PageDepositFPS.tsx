  ...
  return (
    <>
      ...
      <div style={{ backgroundColor: 'white', paddingLeft: '5%', paddingRight: '5%', paddingTop: '40px' }}>
        ...
        <h1 style={{ fontSize: '40px', color: 'grey' }}>{g(k.data, 'text_1')}</h1>
          {
              k.procedure.map((j:any, i:any) => {
                return (
                    <div style={{display:'flex',flexDirection:'row',alignItems:'center'}}  key={i}>
                      ...
                      <p style={{fontSize:'15px'}}>{z(j, 'title')}</p>
                    </div>
                )
              })
            }
        ...
        <h1 style={{ fontSize: '20px', color: 'grey', paddingTop: '40px' }} dangerouslySetInnerHTML={{ __html: g(k.data, 'note') }}></h1>
      </div >
