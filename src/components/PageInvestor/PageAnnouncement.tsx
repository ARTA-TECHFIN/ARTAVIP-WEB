...
const getAnnouncementAndNotices= async() => {
  const res = await fetch(`${process.env.NEXT_PUBLIC_HOSTING_PATH}/api/cms/announcements-and-noticess?pagination%5BpageSize%5D=500`)
...
const useGetData = (locale: string) => {
  ...
  return useQuery([QUERY_ANNOUNCEMENT, lang, year], async () => {
    const result = await getAnnouncementAndNotices();
      ...
      {data?.map((yearly, index) => (
        <CalendarAccordion
          ...
          events={yearly.results.map((r: any) => ({
            ...
            url: locale === 'en'? r.attributes.pdf_en: locale === 'tc'? r.attributes.pdf_tc :r.attributes.pdf_sc,
          }))}
          openYear={openYear}
