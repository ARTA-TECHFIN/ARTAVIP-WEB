...
const PageEsg: NextPage = () => {
  ...
  return (
    <div className='grid grid-cols-12 gap-x-5 pt-16'>
      <ul ref={nav} className='col-span-3 !translate-y-0 h-[140px]'>
        {
          navList.map((item, index) => (
            <li className={`cursor-pointer text-black mb-6 flex items-center ${textClass.body_regular_verah}`} 
              key={index} 
      ...
      <div className='col-span-9' ref={sections}>
        <div  id='introduction' className='pb-16 esg-section'>
        ...
        <div id="environmental" className='esg-section'>
          ...
          <p className={`mt-4 ${textClass.body_regular_verah}`}>{`Acting responsibly to our planet earth, we devote great efforts to preventing pollution, minimizing energy consumption and reducing physical waste. At ARTA TechFin’s offices, we promote environmentally responsive virtue. Staff at our office are encouraged to have their recyclable waste categorized by paper, metals and plastics at our recycling facilities.`}</p>
          <p className={`mt-4 ${textClass.body_regular_verah}`}>{`We have our offices located in K11 Atelier certified green offices. As an environmentally responsible tenant, we have an eco-lighting system installed to ensure electricity will be utilized only when it is necessary at the workstations and rooms; water-saving taps are installed and water will run with a lot more bubbles so that net water volume will be pared. By 2025, we aim to gradually reduce our electricity usage per square foot by 40% and water usage by 50% compared to 2021. `}</p>
          <p className={`mt-4 ${textClass.body_regular_verah}`}>{`We also take a prudent approach to ensure our business is not a threat to the environment. In every investment strategy and service provider selection, we shall take sustainability into consideration.`}</p>
        </div>

        <div id='social' className='pt-16 esg-section'>
          <h2 className={`${textClass.h3_style2}`}>{`Social`}</h2>
          <h3 className={`mt-4 text-arta-sand-200 ${textClass.h6}`}>{`Employee care`}</h3>
          <p className={`mt-4 ${textClass.body_regular_verah}`}>{`To nurture an equal, engaged and productive working environment, we have adopted flexible working hours. Our performance evaluation is result oriented so that staff can utilize their hours to achieve work-life balance.`}</p>
          <p className={`mt-4 ${textClass.body_regular_verah}`}>{`We respect and stand up for the rights and dignity of staff of different races, genders and sexual orientations. People at ARTA TechFin are treated equally and fairly. We evaluate our staff by performance instead of identity.`}</p>
          <p className={`mt-4 ${textClass.body_regular_verah}`}>{`Regarding talent management, we adhere to a ‘people-oriented’ principle while striving to provide an equal, diverse and respectful work environment for our employees. We have eliminated any trace of discrimination in hiring or treatment regardless of race, religion, gender, age, marital status, or physical and mental disability. To encourage greater workplace diversity and attract global talent from all walks of life, we welcome suitable foreign applicants and encourage employees to work from home remotely from different countries.`}</p>
          <h3 className={`mt-4 text-arta-sand-200 ${textClass.h6}`}>{`Community`}</h3>
          <p className={`mt-4 ${textClass.body_regular_verah}`}>{`We believe corporations should take responsibility for fostering a sustainable community, given their considerable influence and prominence within society. We continue to show our care for different aspects of local community development, especially in the focus areas of employment promotion, social services, and community technology development.`}</p>
          <p className={`mt-4 ${textClass.body_regular_verah}`}>{`We encourage our employees to participate in charitable events to contribute to society. The company will also organize charitable events as well as monetary donations to charities that match ARTA TechFin’s values.`}</p>
          <p className={`mt-4 ${textClass.body_regular_verah}`}>{`Sharing knowledge to the youth and people in need generously is our social obligation. We will organize talks, seminars and courses with different institutions every year. We will offer up-to-date technical and finance know-how, case studies and market information to the interested parties.`}</p>
        </div>
       
        
       <div id='Governance' className='pt-16 esg-section'>
        <h2 className={`${textClass.h3_style2}`}>{`Governance`}</h2>
        <p className={`mt-4 ${textClass.body_regular_verah}`}>{`We believe the growth and sustainability of the Group's businesses depend on the maintenance of high standards of corporate governance and business ethics. We value transparency and accountability as the keys to achieving a high standard of corporate governance, earning the confidence of shareholders and the public. In order to achieve this, ARTA TechFin has implemented strict governance procedures to guarantee accountability and transparency for our shareholders' long-term interests.`}</p>

        <h3 className={`mt-4 text-arta-sand-200 ${textClass.h6}`}>{`The Board`}</h3>
        <p className={`mt-4 ${textClass.body_regular_verah}`}>{`The Board currently comprises a total of eight Directors, being three Executive Directors, two Non-Executive Directors and three Independent Non-Executive Directors, with expertise and experience covering a wide range of professions. The number of Independent Non-Executive Directors represents more than one-third of the Board. They have actively participated in the board committees of the Company and have made significant contributions of their skills and expertise to these committees.`}</p>
