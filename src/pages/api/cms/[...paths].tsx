...
export default async function handler(
  ...
  let url;
  if (queryStr) {
    // url = `${process.env.ARTAVIP_CMS_API_ENDPOINT}/` + paths.join("/") + "?" + queryStr;
    url = `http://127.0.0.1:1347/api/` + paths.join("/") + "?" + queryStr;
  } else {
    // url = `${process.env.ARTAVIP_CMS_API_ENDPOINT}/` + paths.join("/");
    url = `http://127.0.0.1:1347/api/` + paths.join("/");
  }
  try {
